...
volatile __xdata uint8_t HidCommandReceived = 0;
volatile __xdata uint8_t UpPoint2_Busy = 0;

void raw_hid_receive(void) { HidCommandReceived = 1; }

uint8_t raw_hid_received(void) {
    if (HidCommandReceived == 1) {
        HidCommandReceived = 0;
        return 1;
    }
    return 0;
}
